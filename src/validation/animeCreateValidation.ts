...
export const animeCreateValidation = (state: AnimeCreateEntity): string[] => {
    const errors: string[] = [];
    const { animeImagesPreview, epizodeDuration, epizodesCount, hours, kind, minutes, productionYear, scenario, soundtracks, soundtracksPreview, title, types, watchLink, background, baner, mini } = state;
    ...
    if (!background) {
        errors.push('Wybierz grafikę na tło anime.');
    }

    if (!baner) {
        errors.push('Wybierz grafikę na baner anime.');
    }

    if (!mini) {
        errors.push('Wybierz grafikę na okładkę anime.');
    }

    if (animeImagesPreview?.background.size > 524288 || animeImagesPreview?.baner.size > 524288 || animeImagesPreview?.mini.size > 524288) {
        errors.push('Grafiki nie powinny przekraczać 0.5MB.');
    }
