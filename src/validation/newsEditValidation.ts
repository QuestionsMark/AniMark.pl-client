...
export const newsEditValidation = (form: NewsEditEntity, prevState: NewsEditEntity): string[] => {
    const errors: string[] = [];
    const { choosedImages, description, images, otherLinks, preview, title, videos, savedImages } = form;
    ...
    if (choosedImages.length + preview.length + savedImages.length > 5) {
    ...
    if (
        ...
        JSON.stringify(videos) === JSON.stringify(prevState.videos) &&
        JSON.stringify(otherLinks) === JSON.stringify(prevState.otherLinks) &&
        choosedImages.length + preview.length + savedImages.length === savedImages.length
    ) {
